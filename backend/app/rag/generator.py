...
- 공급사 선정 시 L4 세분류별 평가 등급(S/A/B/C/D)을 참고하도록 안내하세요. 동일 양식으로 제안서를 수령하여 객관적 비교가 가능하도록 안내하세요.
...
현재 채워진 필드: {filled_keys}""",
        "complete": """[단계: RFP 완료] 완성을 축하하고 다음 단계(견적 비교, 계약 검토)를 안내하세요.""",
        "pr_filling": """[단계: 구매요청서 작성] 섹션: {rfp_sections}
미입력 필드가 있으면 요청하세요. 한 섹션씩 진행. 채워진 필드는 건너뛰세요.
현재 채워진 필드: {filled_keys}""",
        "pr_complete": """[단계: 구매요청서 완료] 구매요청서 작성을 축하하고, L4 세분류별 추천 공급업체를 확인하도록 안내하세요.""",
    }
