...
3. 쉼표로 구분된 값은 AI가 요청한 필드 순서대로 1:1 매핑하세요.
   예: AI가 "요구 사양, 포함 서비스, 유지보수 기준, 반납·인수 조건"을 요청했고
   사용자가 "블랙박스 추가, 엔진오일 10회 무료, 48개월, 반납조건"이라 답하면
   → s11=블랙박스 추가, s12=엔진오일 10회 무료, s13=48개월, s14=반납조건
4. 값의 개수와 요청 필드 개수가 같으면 반드시 순서대로 1:1 매핑하세요. 하나의 필드에 여러 값을 합치지 마세요.
