...
// L4 공급업체 등급 색상
const GRADE_COLORS = { S: '#D97706', A: '#059669', B: '#2563EB', C: '#6B7280', D: '#DC2626' };
const GRADE_BG = { S: '#FEF3C7', A: '#D1FAE5', B: '#DBEAFE', C: '#F3F4F6', D: '#FEE2E2' };
...
export default function ChatPage() {
  ...
  const [activeRfpFieldKey, setActiveRfpFieldKey] = useState(null);  // RFP 자율답변 대상 필드
  const [rfpUserFilledKeys, setRfpUserFilledKeys] = useState(new Set());
  const [rfpFillingTurns, setRfpFillingTurns] = useState(0);
  const [rfpManualClosed, setRfpManualClosed] = useState(false);
  ...
  // L4 공급업체 추천
  const [l4Code, setL4Code] = useState(null);
  const [l4Options, setL4Options] = useState([]);
  const [l4Branch, setL4Branch] = useState(null);
  const [l4ScopeType, setL4ScopeType] = useState('nationwide');
  const [l4ScopeValue, setL4ScopeValue] = useState(null);
  const [l4Suppliers, setL4Suppliers] = useState({ fixed: [], rotating: [], eval_criteria: [] });
  ...
  // RFP 사용자 채움 추적 (PR/RFQ와 동일 패턴: 30%/3턴 자동오픈)
  const RFP_SKIP_KEYS_CALC = new Set(["s1","s2","s3","s4","s5"]); // 발주기관 정보 제외
  const RFP_AUTO_OPEN_PCT = 30;
  const rfpRequiredFields = Object.entries(fields).filter(([k]) => !RFP_SKIP_KEYS_CALC.has(k));
  const rfpRequiredTotal = rfpRequiredFields.length;
  const rfpUserFilledPct = rfpRequiredTotal > 0
    ? Math.round(rfpRequiredFields.filter(([k]) => rfpUserFilledKeys.has(k)).length / rfpRequiredTotal * 100) : 0;
  ...
  // RFP 패널 자동 오픈: 30% 이상 또는 3턴 이상 (PR/RFQ와 동일)
  useEffect(() => {
    if (phase === "filling" && !rightVisible && !rfpManualClosed) {
      if (rfpUserFilledPct >= RFP_AUTO_OPEN_PCT || rfpFillingTurns >= 3) {
        setRightVisible(true);
      }
    }
  }, [rfpUserFilledPct, rfpFillingTurns, phase, rightVisible, rfpManualClosed]);
  ...
  const applyFills = (fills) => {
    ...
    setJustFilled(keys);
    setRfpUserFilledKeys(prev => new Set([...prev, ...Object.keys(fills)]));
    setFields(prev => {
      const next = { ...prev };
  ...
  // ── L4 공급업체 추천 ──
  const fetchL4Suppliers = async (code, scopeType = 'nationwide', scopeValue = null) => {
    setL4Loading(true);
    try {
      const res = await api.getL4Suppliers(code, scopeType, scopeValue, sessionId);
      setL4Suppliers({
        fixed: res.fixed || [],
        rotating: res.rotating || [],
        eval_criteria: res.eval_criteria || [],
      });
      setL4Branch(res.branch || null);
    } catch (e) {
      console.warn('L4 supplier fetch failed:', e);
    } finally {
      setL4Loading(false);
    }
  };
  ...
  const handleL4ScopeSelect = (scopeType, scopeValue) => {
    setL4ScopeType(scopeType);
    setL4ScopeValue(scopeValue);
    if (l4Code) fetchL4Suppliers(l4Code, scopeType, scopeValue);
  };
  ...
  const handleRfpTypeSelect = (type) => {
    ...
    setRightVisible(false);      // 대화로 수집 후 30%/3턴 자동오픈 (PR/RFQ 동일)
    setRfqRightVisible(false);
    setPrRightVisible(false);
    setRfpUserFilledKeys(new Set());
    setRfpFillingTurns(0);
    setRfpManualClosed(false);
    setActiveRfpFieldKey(null);
    setPhase("filling");
  ...
  const handleSend = async (directText) => {
    ...
    try {
      ...
      if (phase === "pr_filling" && activePrFieldKey) {
        ...
        const selected = selectedSupplierIds.size > 0
          ? allSuppliers.filter(s => selectedSupplierIds.has(s.id || `${s.company}_${allSuppliers.indexOf(s)}`))
          : allSuppliers;
        const data = await api.chat(sessionId, text, null, history, phase, {}, rfpType, prType, getPrFilledFields(), userRole, roleTurnCount, null, {}, lastClassification?.l3_code || null, l4Code, l4NameFound, selected);
        ...
        // RFP 자율답변 모드: activeRfpFieldKey가 있으면 직접 매핑 (PR/RFQ 동일)
        if (activeRfpFieldKey && fields[activeRfpFieldKey]) {
          applyFills({ [activeRfpFieldKey]: text });
          setRfpFillingTurns(prev => prev + 1);
          const fieldLabel = fields[activeRfpFieldKey]?.label || activeRfpFieldKey;
          setActiveRfpFieldKey(null);
          setMessages(prev => [...prev, {
            id: msgIdCounter++, role: "assistant",
            text: `**${fieldLabel}** 입력 완료. 다음 항목을 입력해 주세요.`,
        ...
        await api.streamChat(
          ...
          (meta) => {
            ...
            if (meta._l4_event === "l4_select") {
              ...
              if (meta.message) {
                setMessages(prev => [...prev, {
                  id: msgIdCounter++, role: "assistant", text: meta.message,
                  isL4Guide: true,
                }]);
              }
              return;
            }
            if (meta._l4_event === "l4_branch") {
              setL4Code(meta.l4_code);
              setL4Branch(meta);
              fetchL4Suppliers(meta.l4_code);
              if (meta.message) {
                setMessages(prev => [...prev, {
                  id: msgIdCounter++, role: "assistant", text: meta.message,
                  isL4Guide: true,
  ...
  // ── RFP 인라인 탭/퀵필 (PR/RFQ와 동일 패턴) ──
  const RFP_SKIP_KEYS = new Set(["s1","s2","s3","s4","s5"]); // 발주기관 정보 → 인라인탭 제외

  const renderRfpInlineTabs = () => {
    if (!rfpType || !fields || Object.keys(fields).length === 0) return null;
    const unconfirmed = Object.entries(fields)
      .filter(([k, f]) => !RFP_SKIP_KEYS.has(k) && !rfpUserFilledKeys.has(k));
    if (unconfirmed.length === 0) return null;
    const withOpts = unconfirmed.filter(([k, f]) => getRfpFieldOptions(k, f).length > 0);
    const [fk, f] = withOpts.length > 0 ? withOpts[0] : unconfirmed[0];
    const opts = getRfpFieldOptions(fk, f);
    const currentVal = (f.value || "").trim();
    return (
      <div style={{ marginTop:10 }}>
        <div style={{ fontSize:10, fontWeight:600, color: T.sub, marginBottom:6 }}>
        ...
        <div style={{ background:"rgba(6,182,212,0.04)", border:"1px solid rgba(6,182,212,0.12)", borderRadius:10, padding:"8px 12px" }}>
          <div style={{ fontSize:11, fontWeight:700, color: T.primary, marginBottom:5, display:"flex", alignItems:"center", gap:4 }}>
            {f.label}
            {currentVal && <span style={{ fontSize:10, fontWeight:500, color: T.sub, marginLeft:"auto" }}>현재: {currentVal}</span>}
          </div>
          <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
            {opts.map((opt, i) => (
              <button key={i} onMouseDown={(e) => {
                ...
                applyFills({ [fk]: opt });
                setRfpFillingTurns(prev => prev + 1);
                setMessages(prev => [...prev, { id: msgIdCounter++, role:"assistant", text:`**${f.label}**: ${opt}`, rfpInlineTabs:true, isQuickFillConfirm:true }]);
              }} style={{
                padding:"5px 11px", borderRadius:14, fontSize:11, fontWeight:600,
                border:`1px solid ${opt===currentVal ? T.primary : "rgba(6,182,212,0.2)"}`,
                background: opt===currentVal ? "rgba(14,165,160,0.1)" : "rgba(255,255,255,0.8)",
            ...
            <button onMouseDown={(e) => {
              e.preventDefault(); e.stopPropagation();
              setActiveRfpFieldKey(fk);
              inputRef.current?.focus();
            }} style={{
              padding:"5px 11px", borderRadius:14, fontSize:11, fontWeight:600,
              border:`1px dashed ${activeRfpFieldKey===fk ? T.primary : "rgba(100,116,139,0.3)"}`,
              background: activeRfpFieldKey===fk ? "rgba(14,165,160,0.08)" : "transparent",
              color: activeRfpFieldKey===fk ? T.primary : T.muted,
              cursor:"pointer", fontFamily:"inherit", transition:"all 0.15s",
            }}>직접 입력</button>
          </div>
        </div>
  ...
  const renderRfpQuickFillCards = () => {
    if (!rfpType || !fields || Object.keys(fields).length === 0) return null;
    const unfilledFields = Object.entries(fields)
      .filter(([k, f]) => !RFP_SKIP_KEYS.has(k) && !(f.value || "").trim());
    if (unfilledFields.length === 0) return null;
    const withOpts = unfilledFields.filter(([k, f]) => getRfpFieldOptions(k, f).length > 0);
    const [fk, f] = withOpts.length > 0 ? withOpts[0] : unfilledFields[0];
    const opts = getRfpFieldOptions(fk, f);
    return (
      <div style={{ marginTop:12 }}>
        <div style={{ fontSize:10, color: T.sub, marginBottom:6 }}>필수 항목 {unfilledFields.length}개 남음</div>
        <div style={{ background:"rgba(6,182,212,0.04)", border:"1px solid rgba(6,182,212,0.12)", borderRadius:10, padding:"10px 14px" }}>
          <div style={{ fontSize:11, fontWeight:700, color: T.primary, marginBottom:6, display:"flex", alignItems:"center", gap:4 }}>
            {f.label}
          </div>
          <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
  ...
  const [rfqTab, setRfqTab] = useState(0);
  const RfqPanelFilling = () => {
    // 공급업체 영역 제외한 탭 목록
    const nonSupplierSecs = rfqSections.filter(s => !s.supplier_zone);
    const supplierSecs = rfqSections.filter(s => s.supplier_zone);
    const currentSec = nonSupplierSecs[rfqTab] || nonSupplierSecs[0];

    return (
    <div className="custom-scroll" style={{ flex:1, overflowY:"auto", padding:"16px 18px" }}>
      {/* 진행률 바 */}
      <div style={{
        background: `linear-gradient(135deg, rgba(14,165,160,0.06), rgba(6,182,212,0.04))`,
        borderRadius: T.r12, padding:"12px 16px", marginBottom:12,
        border: `1px solid rgba(14,165,160,0.12)`,
      }}>
      ...
      <div style={{ display:"flex", gap:4, marginBottom:12, overflowX:"auto", paddingBottom:4 }}>
        {nonSupplierSecs.map((sec, ti) => {
          const isActive = rfqTab === ti;
          const done = sec.fields.every(fk => (rfqFields[fk]?.value || "").trim());
          const cnt = sec.fields.filter(fk => (rfqFields[fk]?.value || "").trim()).length;
          const shortTitle = sec.title.replace(/^\d+\.\s*/, "").replace(/^※\s*/, "");
          return (
            <button key={ti} onMouseDown={(e) => { e.preventDefault(); setRfqTab(ti); }} style={{
              padding:"8px 12px", borderRadius: T.r8, fontSize:11, fontWeight:isActive ? 700 : 500,
              border: isActive ? `1.5px solid ${T.primary}` : `1px solid ${T.border}`,
              background: isActive ? "rgba(14,165,160,0.08)" : T.card,
              color: isActive ? T.primary : T.sub,
              cursor:"pointer", fontFamily:"inherit", whiteSpace:"nowrap", transition:"all 0.15s",
              display:"flex", alignItems:"center", gap:4, flexShrink:0,
            }}>
              {done ? <span style={{ color:T.greenDark, fontSize:9 }}>✓</span> : null}
              {shortTitle}
              <span style={{ fontSize:9, color: isActive ? T.primary : T.muted }}>{cnt}/{sec.fields.length}</span>
            </button>
          );
        })}
      </div>
      ...
      {currentSec && (
        <div style={{
          background: 'rgba(255,255,255,0.65)', borderRadius: T.r12,
          border: `1px solid rgba(14,165,160,0.08)`, overflow:"hidden", marginBottom:12,
        }}>
          {currentSec.fields.map((fk, fi) => {
            const f = rfqFields[fk];
            if (!f) return null;
            const isEmpty = !(f.value || "").trim();
            const isRequired = f.required !== false;
            const opts = isEmpty && isRequired ? getRfqFieldOptions(fk, f) : [];
            return (
              <div key={fk}>
                <div style={{
                  display:"flex",
                  borderBottom: fi < currentSec.fields.length - 1 ? `1px solid ${T.borderLight}` : "none",
                  background: fi % 2 === 1 ? "rgba(14,165,160,0.02)" : "transparent",
                }}>
                  <div style={{
                    width:120, padding:"10px 12px", flexShrink:0,
                    background:"rgba(14,165,160,0.03)", borderRight:`1px solid rgba(14,165,160,0.06)`,
                    fontSize:11, fontWeight:700, color: T.sub, display:"flex", alignItems:"center", minHeight:38,
                  }}>
                    {f.label}
                    {isRequired && <span style={{ color:T.red, marginLeft:2 }}>*</span>}
                  </div>
                  <div style={{ flex:1, padding:"4px 8px", display:"flex", alignItems:"center", minHeight:38 }}>
                    <input
                      value={f.value || ""}
                      onChange={e => {
                        setRfqFields(prev => ({ ...prev, [fk]: { ...prev[fk], value: e.target.value } }));
                        setRfqUserFilledKeys(prev => new Set([...prev, fk]));
                      }}
                      placeholder="직접 입력 또는 채팅"
                      style={{
                        width:"100%", border:"none", outline:"none", background:"transparent",
                        fontSize:12, color: T.text, padding:"5px 6px", borderRadius:4, fontFamily:"inherit",
                      }}
                      onFocus={e => e.target.style.background = "rgba(14,165,160,0.06)"}
                      onBlur={e => e.target.style.background = "transparent"}
                    />
                ...
                {/* 퀵필 옵션 칩 — RFP 패턴 동일 */}
                {opts.length > 0 && (
                  <div style={{ padding:"4px 8px 6px 132px", display:"flex", gap:4, flexWrap:"wrap", background: fi % 2 === 1 ? "rgba(14,165,160,0.02)" : "transparent" }}>
                    {opts.map((opt, oi) => (
                      <button key={oi} onMouseDown={(e) => {
                        e.preventDefault();
                        setRfqFields(prev => ({ ...prev, [fk]: { ...prev[fk], value: opt } }));
                        setRfqUserFilledKeys(prev => new Set([...prev, fk]));
                        setRfqFillingTurns(prev => prev + 1);
                      }} style={{
                        padding:"3px 9px", borderRadius:12, fontSize:10, fontWeight:600,
                        border:`1px solid rgba(14,165,160,0.2)`, background:"rgba(255,255,255,0.8)",
                        color: T.text, cursor:"pointer", fontFamily:"inherit", transition:"all 0.12s",
                      }}
                        onMouseEnter={e => { e.currentTarget.style.background="rgba(14,165,160,0.1)"; e.currentTarget.style.borderColor=T.primary; }}
                        onMouseLeave={e => { e.currentTarget.style.background="rgba(255,255,255,0.8)"; e.currentTarget.style.borderColor="rgba(14,165,160,0.2)"; }}
                      >{opt}</button>
                    ))}
                  </div>
                )}
      ...
      {/* 공급업체 영역 (접이식) */}
      {supplierSecs.map((sec, si) => (
        <div key={`sup-${si}`} style={{ marginBottom:8 }}>
          <div onClick={() => setRfqOpenSec(p => ({...p,[`s${si}`]:!p[`s${si}`]}))} style={{
            background:"#FFFBEB", padding:"10px 14px", borderRadius: rfqOpenSec[`s${si}`] ? `${T.r10}px ${T.r10}px 0 0` : T.r10,
            border:"1px solid #FDE68A", display:"flex", alignItems:"center", gap:8, cursor:"pointer",
          }}>
            <span style={{ fontSize:13 }}>📦</span>
            <span style={{ fontSize:11, fontWeight:700, color:"#92400E", flex:1 }}>{sec.title} (공급업체 작성란)</span>
            <span style={{ color:"#B45309", fontSize:11, transform: rfqOpenSec[`s${si}`] ? "rotate(180deg)" : "rotate(0)", transition:"0.2s" }}>▾</span>
          </div>
          {rfqOpenSec[`s${si}`] && (
            <div style={{ background:"rgba(254,243,199,0.1)", border:"1px solid #FDE68A", borderTop:"none", borderRadius:`0 0 ${T.r10}px ${T.r10}px`, overflow:"hidden" }}>
              <div style={{ padding:"8px 14px", background:"#FEF3C7", fontSize:10, color:"#92400E" }}>소싱담당자는 입력하지 않아도 됩니다.</div>
              {sec.fields.map((fk, fi) => {
                const f = rfqFields[fk]; if (!f) return null;
                return (
                  <div key={fk} style={{ display:"flex", borderBottom: fi < sec.fields.length-1 ? "1px solid #FDE68A" : "none" }}>
                    <div style={{ width:120, padding:"8px 12px", background:"rgba(254,243,199,0.2)", borderRight:"1px solid #FDE68A", fontSize:10, fontWeight:600, color:"#92400E", display:"flex", alignItems:"center" }}>{f.label}</div>
                    <div style={{ flex:1, padding:"4px 8px", display:"flex", alignItems:"center" }}>
                      <input value={f.value||""} onChange={e => setRfqFields(prev => ({...prev,[fk]:{...prev[fk],value:e.target.value}}))} placeholder="공급업체 기재란" style={{ width:"100%", border:"none", outline:"none", background:"transparent", fontSize:11, color:"#92400E", padding:"5px 6px", fontFamily:"inherit" }} />
                    </div>
                  </div>
                );
              })}
      ...
      {/* 하단 버튼 */}
      <div style={{ display:"flex", gap:8, marginTop:12 }}>
        <button onClick={() => downloadRfqPdf(rfqFields, rfqSections, rfqTemplate?.name, rfqFields.rq1?.value)} style={{
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div ref={chatScrollRef} className="custom-scroll" style={{
          ...
          {messages.map((msg, mi) => (
            <div key={msg.id} style={{
              ...
              <div style={{ maxWidth:"78%", display:"flex", flexDirection:"column", gap:6 }}>
                  ...
                  <div style={{ marginTop:12 }}>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                      ...
                      >
                        ...
                        <div style={{ fontSize:10, color: T.sub, marginTop:4 }}>구매요청서 작성</div>
                      </button>
                ...
                {/* RFP 퀵필 카드 — 마지막 rfpQuickFill 메시지에만 표시 */}
                {msg.rfpQuickFill && phase === "filling" && mi === messages.findLastIndex(m => m.rfpQuickFill) && renderRfpQuickFillCards()}

                {/* RFP 인라인 탭 — 마지막 rfpInlineTabs 메시지에만 표시 */}
                {msg.rfpInlineTabs && phase === "filling" && mi === messages.findLastIndex(m => m.rfpInlineTabs) && renderRfpInlineTabs()}
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12, padding: '0 16px' }}>
              ...
              {l4Options.map(opt => (
                <button key={opt.code} onClick={() => handleL4Select(opt.code)}
                  style={{
                    padding: '8px 16px', borderRadius: 20, border: '1.5px solid #0ea5a0',
                    background: 'rgba(14,165,160,0.06)', color: '#0ea5a0', fontSize: 13,
          ...
          {/* L4 지역/공종 분기 선택 */}
          {l4Code && l4Branch && (l4Branch.has_region || l4Branch.has_worktype) && l4ScopeType === 'nationwide' && !l4Loading && (
            <div style={{ padding: '0 16px', marginBottom: 12 }}>
              {l4Branch.has_region && (
                <div style={{ marginBottom: 8 }}>
                  <div style={{ fontSize: 13, color: '#64748b', marginBottom: 6 }}>지역 선택 (선택사항)</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {(l4Branch.regions || []).map(r => (
                      <button key={r} onClick={() => handleL4ScopeSelect('regional', r)}
                        style={{
                          padding: '6px 12px', borderRadius: 16, border: '1px solid #06b6d4',
                          background: '#f0fdfa', color: '#0e7490', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit',
                        }}>
                        {r}
                      </button>
                    ))}
              ...
              {l4Branch.has_worktype && (
                <div>
                  <div style={{ fontSize: 13, color: '#64748b', marginBottom: 6 }}>공종 선택</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {(l4Branch.worktypes || []).map(w => (
                      <button key={w} onClick={() => handleL4ScopeSelect('worktype', w)}
                        style={{
                          padding: '6px 12px', borderRadius: 16, border: '1px solid #f59e0b',
                          background: '#fffbeb', color: '#b45309', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit',
                        }}>
                        {w}
                      </button>
                    ))}
            ...
            {phase === "pr_complete" && (
              <>
                ...
                {!prSaved ? (
                  ...
                  <div style={{ marginTop:16, display:"flex", gap:8 }}>
                    ...
                    <button onClick={() => {
                      if (currentPrTemplate) {
                        downloadPrPdf(prFields, currentPrSections, currentPrTemplate.label || currentPrTemplate.name);
                      }
                    }} style={{
                      flex:1, padding:"14px", borderRadius: T.r10,
                      border:`1px solid ${T.border}`, background: T.card,
                  ...
                  /* 저장 후: 미리보기 | PDF 다운로드 | RFP 전환 | RFQ 전환 */
